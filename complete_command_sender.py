...
class KeyboardSimulator:
    ...
    def get_securecrt_version(self, process_id):
        """获取SecureCRT版本信息"""
        try:
            if not PSUTIL_AVAILABLE:
                logger.warning("psutil不可用，无法获取SecureCRT版本信息")
                return "unknown"
            
            process = psutil.Process(process_id)
            exe_path = process.exe()
            logger.info(f"SecureCRT可执行文件路径: {exe_path}")
            ...
            logger.info(f"获取到SecureCRT版本: {version}")
            return version
        except Exception as e:
            logger.error(f"获取SecureCRT版本失败: {e}")
            logger.debug(f"获取SecureCRT版本异常详情: {traceback.format_exc()}")
            return "unknown"
    ...
    def detect_terminal_type(self, hwnd):
        ...
        try:
            ...
            # 获取窗口样式和扩展样式
            window_style = self.win32gui.GetWindowLong(hwnd, self.win32con.GWL_STYLE)
            window_ex_style = self.win32gui.GetWindowLong(hwnd, self.win32con.GWL_EXSTYLE)
            logger.info(f"窗口样式: 0x{window_style:X}, 扩展样式: 0x{window_ex_style:X}")
            ...
            # 同时检查窗口标题、进程名、类名和样式，提高检测准确性
            window_title_lower = window_title.lower()
            process_name_lower = process_name.lower()
            window_class_lower = window_class.lower()
            logger.info(f"窗口标题(小写): {window_title_lower}, 进程名(小写): {process_name_lower}, 类名(小写): {window_class_lower}")
            ...
            # PowerShell检测
                ...
                return "powershell"
            # SecureCRT检测，增强版
            elif ("securecrt" in window_title_lower or "securecrt.exe" in process_name_lower or 
                  "vtwin32" in window_class_lower or "vttitlebar" in window_class_lower or 
                  "vtwin" in window_class_lower):
                # 进一步验证SecureCRT窗口：检查是否存在VTWin32子窗口
                is_securecrt = False
                try:
                    # 枚举子窗口，检查是否有VTWin32类的子窗口
                    def enum_child_proc(child_hwnd, lParam):
                        nonlocal is_securecrt
                        try:
                            child_class = self.win32gui.GetClassName(child_hwnd)
                            if "vtwin32" in child_class.lower():
                                is_securecrt = True
                                logger.info(f"找到SecureCRT特有的VTWin32子窗口: {child_hwnd}, 类名: {child_class}")
                                return False  # 找到后停止枚举
                        except Exception as e:
                            logger.warning(f"枚举子窗口时出错: {e}")
                        return True
                    
                    self.win32gui.EnumChildWindows(hwnd, enum_child_proc, None)
                except Exception as e:
                    logger.warning(f"枚举子窗口失败: {e}")
                
                # 如果直接匹配或找到VTWin32子窗口，确认是SecureCRT
                if ("securecrt.exe" in process_name_lower or is_securecrt or 
                    "securecrt" in window_title_lower and "-" in window_title_lower):
                    # 获取SecureCRT版本信息
                    securecrt_version = "unknown"
                    try:
                        if process_id is None:
                            process_id = win32process.GetWindowThreadProcessId(hwnd)[1]
                        securecrt_version = self.get_securecrt_version(process_id)
                    except Exception as e:
                        logger.warning(f"获取SecureCRT版本失败: {e}")
                    
                    logger.info(f"检测到SecureCRT终端: 标题={window_title_lower}, 进程名={process_name_lower}, 类名={window_class}, 版本={securecrt_version}, 验证VTWin32子窗口={is_securecrt}")
                    # 存储版本信息，供后续发送命令使用
                    self.securecrt_version = securecrt_version
                    return "securecrt"
            # MobaXterm检测
        ...
        try:
            ...
            def is_valid_window(handle):
                ...
                if not self.win32gui.IsWindow(handle):
                    logger.error(f"SecureCRT窗口句柄无效: {handle}")
                    return False
                
                if not self.win32gui.IsWindowVisible(handle):
                    logger.warning(f"SecureCRT窗口不可见: {handle}")
                    # 尝试显示窗口
                    try:
                        self.win32gui.ShowWindow(handle, self.win32con.SW_SHOW)
                        time.sleep(0.1)
                        # 再次检查是否可见
                        if not self.win32gui.IsWindowVisible(handle):
                            logger.error(f"无法使SecureCRT窗口可见: {handle}")
                            return False
                        logger.info(f"成功显示SecureCRT窗口: {handle}")
                    except Exception as e:
                        logger.error(f"显示SecureCRT窗口失败: {e}")
                        return False
                
                # 检查窗口是否启用
                if not self.win32gui.IsWindowEnabled(handle):
                    logger.warning(f"SecureCRT窗口已禁用: {handle}")
                    # 尝试启用窗口
                    try:
                        self.win32gui.EnableWindow(handle, True)
                        time.sleep(0.1)
                        # 再次检查是否启用
                        if not self.win32gui.IsWindowEnabled(handle):
                            logger.error(f"无法启用SecureCRT窗口: {handle}")
                            return False
                        logger.info(f"成功启用SecureCRT窗口: {handle}")
                    except Exception as e:
                        logger.error(f"启用SecureCRT窗口失败: {e}")
                        return False
            ...
            if not is_valid_window(hwnd):
                logger.error(f"SecureCRT主窗口无效: {hwnd}")
                return False
            ...
            try:
                ...
                if self.win32gui.GetForegroundWindow() != hwnd:
                    ...
                    time.sleep(0.1)
                    # 先发送WM_SETFOCUS消息
                    self.win32gui.PostMessage(hwnd, self.win32con.WM_SETFOCUS, 0, 0)
                    time.sleep(0.1)
                    # 再尝试设置前台窗口
            ...
            # 1. 增强版递归查找SecureCRT的实际终端输入窗口
            def find_terminal_input_window(parent_hwnd):
                """递归查找SecureCRT的终端输入窗口"""
                terminal_windows = []
                
                def enum_child_proc(child_hwnd, lParam):
                    # 检查子窗口是否有效
                    if not self.win32gui.IsWindow(child_hwnd):
                        return True
                    
                    class_name = self.win32gui.GetClassName(child_hwnd)
                    class_name_lower = class_name.lower()
                    logger.debug(f"找到子窗口: {child_hwnd}, 类名: {class_name}")
                    
                    # 查找VTWin32类名的窗口，这是SecureCRT的终端输入窗口
                    if "vtwin32" in class_name_lower or "terminal" in class_name_lower or "console" in class_name_lower:
                        terminal_windows.append(child_hwnd)
                        logger.debug(f"找到SecureCRT终端输入窗口: {child_hwnd}, 类名: {class_name}")
                    # 继续递归查找子窗口
                    self.win32gui.EnumChildWindows(child_hwnd, enum_child_proc, None)
                    return True
                
                self.win32gui.EnumChildWindows(parent_hwnd, enum_child_proc, None)
                
                # 如果找到多个终端窗口，返回第一个
                if terminal_windows:
                    logger.info(f"找到 {len(terminal_windows)} 个SecureCRT终端窗口，使用第一个: {terminal_windows[0]}")
                    return terminal_windows[0]
                else:
                    # 如果没有找到子终端窗口，使用父窗口
                    logger.info(f"未找到SecureCRT子终端窗口，使用父窗口: {parent_hwnd}")
                    return parent_hwnd
            
            # 查找实际的终端输入窗口
            actual_hwnd = find_terminal_input_window(hwnd)
            
            # 验证找到的终端输入窗口
            if not is_valid_window(actual_hwnd):
                logger.error(f"找到的SecureCRT终端输入窗口无效: {actual_hwnd}")
                return False
            
            if actual_hwnd != hwnd:
                logger.info(f"找到SecureCRT终端输入窗口: {actual_hwnd} (父窗口: {hwnd})")
                # 确保终端输入窗口获得焦点
                try:
                    logger.info(f"尝试将焦点设置到SecureCRT终端输入窗口: {actual_hwnd}")
                    self.win32gui.SetFocus(actual_hwnd)
                    time.sleep(0.1)
                    logger.info(f"成功将焦点设置到SecureCRT终端输入窗口")
                except Exception as e:
                    logger.warning(f"设置SecureCRT终端输入窗口焦点失败: {e}")
            else:
                logger.info(f"使用SecureCRT窗口: {actual_hwnd}")
            
            # 初始化默认延迟值 - 降低延迟
            base_delay = 0.05
            logger.info(f"使用SecureCRT默认延迟: {base_delay} 秒")
            
            # 对于长命令，保持相同延迟
            delay = base_delay
            logger.info(f"SecureCRT发送延迟设置: {delay} 秒 (命令长度: {len(text)}字符)")
            
            # 发送逻辑
            win32api = self.win32api
            win32con = self.win32con
            send_message = win32api.SendMessage  # 使用SendMessage确保消息被立即处理
            ...
            # 根据SecureCRT版本调整发送策略
            securecrt_version = getattr(self, 'securecrt_version', 'unknown')
            logger.info(f"使用SecureCRT版本特定发送策略: 版本={securecrt_version}")
            
            # 版本解析，提取主版本号
            main_version = 0
            try:
                main_version = int(securecrt_version.split('.')[0]) if securecrt_version != 'unknown' else 0
                logger.info(f"解析SecureCRT主版本号: {main_version}")
            except Exception as e:
                logger.warning(f"解析SecureCRT版本失败: {e}")
                main_version = 0
            
            # 优化SendMessage参数设置
            def build_lparam_keyevent(vk_code, scan_code=0, extended=False, prev_key_down=False):
                """构建按键事件的lParam参数"""
                lparam = 0
                # 重复计数（1次）
                lparam |= 1 << 0
                ...
                if prev_key_down:
                    lparam |= 1 << 30
                # 转换状态（按下为0，释放为1，但WM_KEYDOWN时应为0）
                lparam |= 0 << 31
                return lparam
            
            def build_lparam_charevent(char_code, scan_code=0, repeat_count=1):
                """构建字符事件的lParam参数，兼容不同版本的SecureCRT"""
                lparam = 0
                # 字符代码（低16位）
                lparam |= (char_code & 0xFFFF) << 0
                # 重复计数（高16位的低8位）
                lparam |= (repeat_count & 0xFFFF) << 16
                # 扫描码（高16位的高8位）
                lparam |= (scan_code & 0xFF) << 24
                # 使用键码作为扫描码，确保SecureCRT能正确识别
                if scan_code == 0:
                    scan_code = char_code
                    lparam |= (scan_code & 0xFF) << 24
                return lparam
            ...
            def char_to_vk(char):
                ...
                if char_lower >= 'a' and char_lower <= 'z':
                    return ord(char_lower.upper())  # A-Z: 0x41-0x5A
                elif char >= '0' and char <= '9':
                    return ord(char)  # 0-9: 0x30-0x39
                elif char == ' ':
                    return 0x20  # 空格键
                elif char == '!':
                    return 0x31  # 1键
            ...
            def char_to_scan_code(char):
                ...
                return scan_codes.get(char_lower, 0)
            
            # 发送每个字符的完整键盘事件序列
            logger.info(f"开始向SecureCRT终端输入窗口 {actual_hwnd} 发送命令: {text!r}")
            
            for i, char in enumerate(text):
                logger.debug(f"SecureCRT发送第 {i+1}/{len(text)} 个字符: {char!r}")
                
                # 获取字符的虚拟键码和扫描码
                vk_code = char_to_vk(char)
                scan_code = char_to_scan_code(char)
                char_code = ord(char)
                
                logger.debug(f"字符 {char!r}: 虚拟键码={vk_code} (0x{vk_code:02X}), 扫描码={scan_code} (0x{scan_code:02X}), 字符码={char_code} (0x{char_code:02X})")
                
                # 1. 发送WM_KEYDOWN消息
                keydown_lparam = build_lparam_keyevent(vk_code, scan_code)
                logger.debug(f"发送WM_KEYDOWN: VK={vk_code}, ScanCode={scan_code}, lParam={keydown_lparam}")
                send_message(actual_hwnd, wm_keydown, vk_code, keydown_lparam)
                
                # 2. 发送WM_CHAR消息
                char_lparam = build_lparam_charevent(char_code, scan_code)
                logger.debug(f"发送WM_CHAR: Char={char!r}, CharCode={char_code}, lParam={char_lparam}")
                send_message(actual_hwnd, wm_char, char_code, char_lparam)
                
                # 3. 发送WM_KEYUP消息
                keyup_lparam = build_lparam_keyevent(vk_code, scan_code)
                keyup_lparam |= (1 << 31)  # 设置释放标志
                logger.debug(f"发送WM_KEYUP: VK={vk_code}, lParam={keyup_lparam}")
                send_message(actual_hwnd, wm_keyup, vk_code, keyup_lparam)
    ...
    def send_enter_securecrt(self, hwnd):
        """SecureCRT特定回车键发送策略 - 增强版，确保可靠性"""
        try:
            VK_RETURN = 0x0D
            logger.info(f"开始向SecureCRT窗口发送回车键: {hwnd}")
            
            # 确保win32api和win32con已正确加载
            if not self.win32api or not self.win32con:
                logger.error(f"SecureCRT win32api或win32con未正确加载，无法发送命令")
                return False
            ...
            # 3. 发送完整的回车键事件序列
            logger.info(f"向SecureCRT发送回车键事件序列")
            
            # 使用SendMessage确保消息被立即处理
            send_message = win32api.SendMessage
            wm_char = win32con.WM_CHAR
            wm_keydown = win32con.WM_KEYDOWN
            wm_keyup = win32con.WM_KEYUP
            ...
            # 发送回车键的完整事件序列
            # 1. 发送WM_KEYDOWN消息
            enter_keydown_lparam = build_lparam_keyevent(VK_RETURN, extended=True)  # 回车键是扩展键
            send_message(actual_hwnd, wm_keydown, VK_RETURN, enter_keydown_lparam)
            sleep(0.02)  # 降低延迟，提高发送速度
            
            # 2. 发送WM_CHAR（回车符）
            enter_char_lparam = build_lparam_charevent(ord('\r'))
            send_message(actual_hwnd, wm_char, ord('\r'), enter_char_lparam)
            sleep(0.02)  # 降低延迟，提高发送速度
            
            # 3. 发送WM_KEYUP消息
            enter_keyup_lparam = enter_keydown_lparam | (1 << 31)  # 释放标志
            send_message(actual_hwnd, wm_keyup, VK_RETURN, enter_keyup_lparam)
            sleep(0.02)  # 降低延迟，提高发送速度
            
            logger.info(f"SecureCRT回车键发送成功")
            return True
        except Exception as e:
            logger.error(f"SecureCRT回车键发送失败: {e}")
            logger.debug(f"SecureCRT回车键发送异常详情: {traceback.format_exc()}")
            return False
...
class CommandSenderApp:
    ...
    def start_drag_select(self):
        ...
        # 临时禁用主窗口的拖拽事件，避免冲突
        self._disable_window_drag_events()
    ...
    def start_mouse_drag_selection(self):
        ...
        try:
            ...
            # 确保重新启用主窗口的拖拽事件
            self._enable_window_drag_events()
    ...
    def select_window_at_position(self, x, y):
        ...
        try:
            ...
            # 更精确的检测逻辑，避免误判
            if ('命令发送器' in window_title and '命令发送器' == window_title[:5]) or \
               ('command_sender' in process_name.lower() and 'command_sender' == process_name.lower()):
    ...
    def _disable_window_drag_events(self):
        """临时禁用主窗口的拖拽事件，避免与拖拽选择功能冲突"""
        try:
            self.root.unbind("<Button-1>")
            self.root.unbind("<B1-Motion>")
            self.root.unbind("<ButtonRelease-1>")
            logger.info("已临时禁用主窗口拖拽事件")
        except Exception as e:
            logger.warning(f"禁用窗口拖拽事件时出错: {e}")
    
    def _enable_window_drag_events(self):
        """重新启用主窗口的拖拽事件"""
        try:
            self.root.bind("<Button-1>", self._on_start_drag)
            self.root.bind("<B1-Motion>", self._on_drag)
            self.root.bind("<ButtonRelease-1>", self._on_stop_drag)
            logger.info("已重新启用主窗口拖拽事件")
        except Exception as e:
            logger.warning(f"启用窗口拖拽事件时出错: {e}")
...
日期：2026-04-21"""
    ...
    def execute_command(self, command):
        ...
        def ensure_window_focus(hwnd):
            ...
            # 检查是否为SecureCRT
            is_securecrt = "securecrt" in window_title.lower() or "securecrt.exe" in process_name_lower or "vtwin32" in window_class.lower() or "vttitlebar" in window_class.lower()
            if is_securecrt:
                # SecureCRT专用焦点获取方法 - 增强版
                def _focus_method_securecrt(hwnd, saved_mouse_pos):
                    """SecureCRT专用焦点获取方法 - 增强版"""
                    logger.info(f"使用SecureCRT专用焦点获取方法")
                    
                    try:
                        # 1. 验证窗口有效性
                        if not win32gui.IsWindow(hwnd):
                            logger.warning(f"SecureCRT窗口 {hwnd} 无效")
                            return False
                        
                        # 2. 检查窗口是否可见
                        is_visible = win32gui.IsWindowVisible(hwnd)
                        if not is_visible:
                            logger.info(f"SecureCRT窗口 {hwnd} 不可见，尝试显示")
                        ...
                        # 5. 发送WM_SETFOCUS消息到SecureCRT
                        win32gui.PostMessage(hwnd, win32con.WM_SETFOCUS, 0, 0)
                        time.sleep(0.05)
                        
                        # 6. 尝试查找并设置焦点到SecureCRT的终端输入窗口
                        def find_terminal_input_window(parent_hwnd):
                            """递归查找SecureCRT的终端输入窗口"""
                            terminal_windows = []
                            ...
                            def enum_child_proc(child_hwnd, lParam):
                                ...
                                if "vtwin32" in class_name.lower():
                                    terminal_windows.append(child_hwnd)
                                # 递归查找子窗口
                                win32gui.EnumChildWindows(child_hwnd, enum_child_proc, None)
                                return True
                            
                            win32gui.EnumChildWindows(parent_hwnd, enum_child_proc, None)
                            return terminal_windows[0] if terminal_windows else parent_hwnd
                        
                        try:
                            terminal_input_hwnd = find_terminal_input_window(hwnd)
                            if terminal_input_hwnd != hwnd:
                                logger.info(f"找到SecureCRT终端输入窗口: {terminal_input_hwnd}")
                                win32gui.SetFocus(terminal_input_hwnd)
                                time.sleep(0.05)
                        except Exception as e:
                            logger.warning(f"查找SecureCRT终端输入窗口失败: {e}")
                        
                        # 7. 验证焦点是否真正获得
                        current_foreground = win32gui.GetForegroundWindow()
                        if current_foreground == hwnd:
                            logger.info(f"SecureCRT窗口 {hwnd} 已获得焦点")
                            return True
                        else:
                            logger.warning(f"SecureCRT窗口 {hwnd} 焦点获取失败，当前前台窗口: {current_foreground}")
                            return False
                            
                    except Exception as e:
                        logger.error(f"SecureCRT焦点获取失败: {e}")
                        logger.debug(f"SecureCRT焦点获取异常详情: {traceback.format_exc()}")
                        return False
                
                focus_methods.append(("SecureCRT专用方法", lambda: _focus_method_securecrt(hwnd, saved_mouse_pos)))
            ...
            # 焦点稳定性检查函数
            def check_focus_stability(target_window, stability_time=0.1):
                """检查窗口焦点是否稳定，在指定时间内保持不变"""
                logger.info(f"开始检查焦点稳定性，持续时间: {stability_time}秒")
                start_time = time.time()
                while time.time() - start_time < stability_time:
                    current_foreground = win32gui.GetForegroundWindow()
                    if current_foreground != target_window:
                        logger.warning(f"焦点在 {time.time() - start_time:.2f}秒内丢失，当前前台窗口: {current_foreground}")
                        return False
                    time.sleep(0.1)  # 每100ms检查一次，减少CPU占用
                logger.info(f"焦点在 {stability_time}秒内保持稳定")
                return True
            
            # 先检查当前窗口是否已经获得焦点
            current_foreground = win32gui.GetForegroundWindow()
            if current_foreground == hwnd:
                logger.info(f"窗口 {hwnd} 已获得焦点，检查焦点稳定性")
                if check_focus_stability(hwnd):
                    logger.info(f"窗口 {hwnd} 焦点稳定，直接返回成功")
                    return True
                else:
                    logger.warning(f"窗口 {hwnd} 焦点不稳定，需要重新获取")
            ...
            for method_name, focus_func in focus_methods:
                try:
                    ...
                    if actual_success:
                        ...
                        # 检查焦点稳定性
                        if check_focus_stability(hwnd):
                            logger.info(f"{method_name} 获取的焦点稳定，返回成功")
                            return True
                        else:
                            logger.warning(f"{method_name} 获取的焦点不稳定，继续尝试其他方法")
                    elif success:
                        # 方法返回成功但实际焦点未获得，记录警告
            ...
            if final_foreground == hwnd:
                logger.info(f"最终检查发现窗口 {hwnd} 已获得焦点，检查焦点稳定性")
                if check_focus_stability(hwnd):
                    logger.info(f"最终焦点稳定，返回成功")
                    return True
                else:
                    logger.warning(f"最终焦点不稳定")
        ...
        # 获取窗口标题和类名，用于更准确的终端类型判断
        window_title = win32gui.GetWindowText(target_hwnd) if WIN32_AVAILABLE else ""
        window_class = win32gui.GetClassName(target_hwnd) if WIN32_AVAILABLE else ""
        logger.info(f"窗口标题: {window_title}, 窗口类名: {window_class}")
        ...
        is_securecrt = terminal_type == "securecrt" or "SecureCRT" in window_title or "VTWin32" in window_class.lower() or "vttitlebar" in window_class.lower()
        ...
        logger.info(f"最终确定终端类型: {terminal_name}, 窗口标题: {window_title}")
        ...
        while send_attempts < max_attempts and not send_success:
            ...
            # 指数退避算法，针对SecureCRT使用更长的初始延迟
            if send_attempts > 0:  # 第一次尝试不需要延迟
                # 针对SecureCRT使用更长的重试延迟
                crt_retry_delay = retry_delay * 2 if is_securecrt else retry_delay
                logger.debug(f"重试延迟: {crt_retry_delay:.3f}秒 (SecureCRT: {is_securecrt})")
                time.sleep(crt_retry_delay)
            ...
            logger.info(f"尝试 {send_attempts}/{max_attempts}: 使用Windows API发送命令: {command_text!r}, 当前焦点窗口: {current_focus}, 焦点一致: {current_focus == target_hwnd}, 终端类型: {terminal_name}, SecureCRT: {is_securecrt}")
        ...
        if not send_success and send_attempts < max_attempts:
            try:
                ...
                # 尝试获取窗口焦点，但即使失败也继续尝试发送命令
                focus_attempt = ensure_window_focus(target_hwnd)
                if not focus_attempt:
                    logger.warning(f"无法获取窗口焦点，但仍尝试剪贴板方式发送命令")
                else:
                    logger.info(f"成功获取窗口焦点，使用剪贴板方式发送命令")
        ...
        if send_success:
            success_msg = f"命令发送成功: {command[:50]}{'...' if len(command) > 50 else ''}"
            self.update_status(success_msg)
            self.sent_count += 1
            self.sent_count_var.set(f"已发送: {self.sent_count}")
            logger.info(f"命令发送成功，总耗时: {total_time:.2f}秒, 尝试次数: {send_attempts}/{max_attempts}")
        else:
            # 所有方式都失败的情况
            error_msg = f"错误: 命令发送失败，所有发送方式均尝试过"
            if final_error:
                error_msg += f" - {final_error}"
            error_msg += f" (尝试次数: {send_attempts}/{max_attempts}, 耗时: {total_time:.2f}秒)"
            self.update_status(error_msg)
            self.failed_count += 1
            self.failed_count_var.set(f"失败: {self.failed_count}")
            logger.error(f"命令发送最终失败，总耗时: {total_time:.2f}秒, 尝试次数: {send_attempts}/{max_attempts}, 错误: {final_error}")
            logger.debug(f"命令发送失败详情: 目标窗口={target_hwnd}, 窗口标题={window_title}, 终端类型={terminal_name}, SecureCRT版本={getattr(self, 'securecrt_version', 'unknown')}")
    ...
    try:
        ...
        if os.path.exists(icon_path):
            try:
                ...
                # 尝试使用另一种方式设置图标
                try:
                    # 尝试将图标转换为PhotoImage并设置
                    icon = tk.PhotoImage(file=icon_path)
                    root.iconphoto(True, icon)
                    print(f"使用iconphoto成功加载图标: {icon_path}")
                except Exception as e2:
                    print(f"使用iconphoto加载图标也失败: {e2}")
        else:
